...
async def test_clip_extreme(session: aiohttp.ClientSession, round_num: int) -> None:
    """Extreme CLIP test."""
...
async def test_llava_if_ready(session: aiohttp.ClientSession, round_num: int) -> None:
    """Test LLaVA if ready."""
    try:
        async with session.get("http://localhost:8003/llava/status", timeout=aiohttp.ClientTimeout(total=2)) as resp:
            if resp.status == 200:
                ...
                if status.get("model_loaded", False):
                    ...
                    print("  LLaVA: Model still loading...")
            else:
                print("  LLaVA: Service error")
    except:
