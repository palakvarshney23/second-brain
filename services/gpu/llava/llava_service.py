...
class LLaVAService(BaseModelService):
    ...
    async def load_model(self) -> None:
        ...
        try:
            ...
            logger.exception(f"Failed to load LLaVA model: {e}")
            raise

    async def generate_embedding(
        self,
        content: str | bytes | Image.Image,
        ...
        try:
                ...
                if isinstance(content, str):
                    ...
                    if LLAVA_AVAILABLE:
                        ...
                        try:
                            ...
                            logger.exception(f"process_images failed: {e}")
                            logger.exception(f"Image: {type(image)}, size: {image.size}")
                            logger.exception(f"Processor: {image_processor}")
                            logger.exception(f"Config: {self.model.config}")
                            raise
            ...
            logger.exception(f"Error generating LLaVA embedding: {e}")
            raise
    ...
    def get_image_processor(self):
        ...
        if self._image_processor is None:
            ...
            try:
                ...
                logger.exception(f"Failed to reload image processor: {e}")
